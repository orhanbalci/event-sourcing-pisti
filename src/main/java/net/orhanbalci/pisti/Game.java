...
public class Game {
    ...
    public void handleEvent(PlayerSeatedEvent playerSeated) {
        ...
        if (gs.getPlayers().getOrElse(List.empty()).length() < 4) {
            publishCommand(new SeatPlayerCommand(gs.getGameId().get(),
    ...
    public void handleEvent(CardsWonEvent cardsWon) {
        ...
        if (cards.length() == 2 && cards.get(0).getNumber() == CardNumber.JACK
        ...
        int tenOfDiamonds = cards.filter(c -> c.getNumber() == CardNumber.TEN && c.getType() == CardType.DIAMONDS)
                .length();
    ...
    public void handleEvent(CardPlayedEvent cardPlayed) {
        ...
        if (gs.getCards().getOrElse(HashMap.empty()).exists((k) -> k._2.length() > 0)) {
            ...
            if (gs.getUndealedCards().getOrElse(List.empty()).isEmpty()) {
                ...
                {
                    publishCommand(new WinCardsCommand(gs.getGameId().getOrNull(), gs.getLastWonPlayer().getOrNull()));
                }else
                {
